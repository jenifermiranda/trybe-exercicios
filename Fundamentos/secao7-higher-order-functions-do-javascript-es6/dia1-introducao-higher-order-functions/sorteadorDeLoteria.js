...
const resultSorteio = (numeroApostado, resultado) => numeroApostado === resultado;

const numeroSorteado = (numeroApostado, callback) => {
  const resultado = Math.floor((Math.random()* 5) +1);

  return callback(numeroApostado, resultado) ? 'Parabens, você ganhou!' : 'Tente Novamente';
}

console.log(numeroSorteado(2, resultSorteio));
